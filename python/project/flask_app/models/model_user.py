...
class User:
    ...
    @classmethod
    def get_one_from_user_id(cls , data:dict)->list:
    ...
    def validate_reg(data:dict)->bool:
        ...
        if len(data['first_name'])<3:
            flash("First name must have more than 3 characters.", "err_reg_first")
            is_valid=False

        if len(data['last_name'])<3:
            flash("Last name must have more than 3 characters.", "err_reg_last")
            is_valid=False
        ...
        if len(data['email'])<2:
            ...
            flash("Invalid email format", "err_reg_em")
            is_valid=False

        if len(data['pw'])<8:
            flash("Password must have more than 8 characters.", "err_reg_pw")
            is_valid=False

        if len(data['confirm_pw'])<8:
            flash("Confirm Password must have more than 8 characters.", "err_reg_cpw")
            is_valid=False
